...
const CallModal = ({
                ...
                if (peerConnection === null) {
                    ...
                    peerConnectionTmp.onicecandidate = ({ candidate }) => {
                        if (candidate) {
                            ...
                            stompClient.send("/app/private-call", {}, JSON.stringify(iceCandidate));
                        }
    ...
    useEffect(() => {
        if (iceCandidate !== null) {
            setIceCandidates(prevCandidates => [...prevCandidates, iceCandidate.candidate]);
        }
    }, [iceCandidate])
    useEffect(() => {
        console.log(answer)
                ...
                footer={[
                    <div key="footer" style={{ textAlign: 'center' }}>
                        ...
                        {isReceiving === true && isTrack === false ? (
                            <Button type="" style={{ color: 'white', backgroundColor: 'green' }} onClick={handleOk}>
                                Pick up
                            </Button>
                        ) : (
                            <span></span>
                        )}
                        <Button type="" style={{ color: 'white', backgroundColor: 'red' }} onClick={handleCancel}>
                            Shutdown
                        </Button>
                    </div>,
                ]}
                ...
                {currentCall !== null ? (
                    <>
                        <div style={{ justifyContent: 'center', display: 'flex' }}>
                            <img src={currentCall.Message_group_image} alt="Image" style={{ borderRadius: '50%', width: '200px', height: '200px' }} />
                            <video id="localVideo" autoPlay playsInline></video>
                            <video id="remoteAudio" autoPlay playsInline ></video>
                        </div>
                        <div>
                            <Button style={{ color: 'white', backgroundColor: 'grey' }}  onClick={toggleCamera}>Toggle Camera</Button>
                            <Button style={{ color: 'white', backgroundColor: 'grey' }} onClick={toggleAudio}>Toggle Audio</Button>
                            <Select
